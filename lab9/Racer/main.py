...
pg.mixer.Sound("Assets/background.wav").play(-1)  # Play background sound in a loop
N = 3
...
backgraund = pg.image.load("Assets/AnimatedStreet.png")
...
Coin_quantity = 0  # Score for quantity coins
Last_coin     = 0  # To fix the issue with spead checking
# Enemy car class
class Enemy(pg.sprite.Sprite):
    def __init__(self):
        super().__init__()
        self.image = pg.image.load("Assets/Enemy.png")  # Load enemy image
...
class Player(pg.sprite.Sprite):
    def __init__(self):
        super().__init__()
        self.image = pg.image.load("Assets/Player.png")  # Load player image
...
class Coin(pg.sprite.Sprite):
    ...
    def move(self):
        ...
        if Coin_quantity % N == 0 and Coin_quantity != 0 and Coin_quantity != Last_coin:
            Espeed += 3 # Increase speed every N coins
            Last_coin = Coin_quantity # To fix issue
...
class SpecialCoin(pg.sprite.Sprite): # Add different weight coin 
    def __init__(self):
        super().__init__()  # Fixed missing parentheses
        self.image = pg.Surface((40, 40), pg.SRCALPHA)
        pg.draw.circle(self.image, Color["red"], (20, 20), 20)  # Draw red circle

        self.rect = self.image.get_rect()
        self.rect.center = (random.randint(40, WIDTH - 40), 0)

    def move(self):
        global Coin_score, Coin_quantity, Espeed, Last_coin # Ensure Coin_score is modified globally
        self.rect.move_ip(0, Espeed)

        # Check if player collects the special coin
        if self.rect.colliderect(P.rect):
            Coin_score += 3 # Add 3 points for special coin
            Coin_quantity += 1 # Increase the total number of collected coins
            self.respawn()

        # If special coin moves out of screen, respawn it
        if self.rect.top > HEIGHT:
            self.respawn()
    ...
    def respawn(self):
        self.rect.top = 0  # Reset position to top
        self.rect.center = (random.randint(40, WIDTH - 40), 0)  # Randomize X position
...
while running:
    ...
    Coin_score_count    = sfont.render("score : " + str(Coin_score), True, Color["black"])
    Coin_quantity_count = sfont.render("coin : " + str(Coin_quantity), True, Color["black"])
    screen.blit(Coin_quantity_count, (130, 10)) # Draw coin quantity
    screen.blit(Coin_score_count, (270, 10))   # Draw coin score
    ...
    if pg.sprite.spritecollideany(P, enemies):
        pg.mixer.Sound("Assets/crash.wav").play()  # Play crash sound
